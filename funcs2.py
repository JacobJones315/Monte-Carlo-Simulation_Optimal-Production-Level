...
uniform_demand_list = np.random.uniform(100,200,1000)
uniform_demand_list = uniform_demand_list.tolist() # creates list of demand instances from array
# convert demand instances to int using for loop
for x in range(0, len(demand_list)):
    demand_list[x] = int(demand_list[x])
    
# convert uniform demand instances to int using for loop
for x in range(0, len(uniform_demand_list)):
    uniform_demand_list[x] = int(uniform_demand_list[x])
...
def compute_profit(manufactured,unit_price,retail_price,disposal_cost):
    ...
    sales_list=[None]*len(demand_list)  #sales list normal distribution
    ...
    uniform_sales_list=[None]*len(uniform_demand_list) # sales list uniform sales list
      
    for x in range(0, len(uniform_demand_list)):
         if uniform_demand_list[x] > manufactured:
              uniform_sales_list[x] = manufactured
         else:
             uniform_sales_list[x] = uniform_demand_list[x]
    ...
    for x in range(0,len(demand_list)):
        distribution = np.random.choice([1,2])
        
        if distribution == 1 and sales_list[x] < manufactured:
            over_production[x] = manufactured - sales_list[x]
            profit[x] = sales_list[x] * (retail_price - unit_price) - (disposal_cost * over_production[x])
        elif distribution == 1 and sales_list[x] > manufactured:
            profit[x] = sales_list[x] * (retail_price - unit_price) 
    
        elif distribution == 2 and uniform_sales_list[x] < manufactured:
            over_production[x] = manufactured - uniform_sales_list[x]
            profit[x] = uniform_sales_list[x] * (retail_price - unit_price) - (disposal_cost * over_production[x])
        elif distribution == 2 and uniform_sales_list[x] > manufactured:
            profit[x] = uniform_sales_list[x] * (retail_price - unit_price) 
        else:
...
manufacture_list = range(100, 200)  # Our optimal value is between 100-200 units
